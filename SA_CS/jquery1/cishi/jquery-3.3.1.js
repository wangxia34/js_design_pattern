...
// enough that all such attempts are guarded in a try block.
    ...
    var
        ...
        //所以其实jquery.fn.init其实返回一个jquery对象 , jquery.fn.init和jquery是一样的。
    ...
    jQuery.extend = jQuery.fn.extend = function () {
        ...
        // 当目标是 string 或 something 时处理（可能在深拷贝中）
        ...
        for (; i < length; i++) {
            ...
            // 只处理非空或者未定义的值 Only deal with non-null/undefined values
            if ((options = arguments[i]) != null) {
                
                // 扩展基本对象 Extend the base object
                for (name in options) {
                    src = target[name];
                    copy = options[name];
                    
                    // 防止陷入死循环 Prevent never-ending loop
                    if (target === copy) {
                        continue;
                    }
                    
                    // 如果我们合并纯对象或数组，则递归 Recurse if we're merging plain objects or arrays
                    if (deep && copy && (jQuery.isPlainObject(copy) ||
                        ...
                        // 不要移动original objects，使用克隆 Never move original objects, clone them
                        target[name] = jQuery.extend(deep, clone, copy);
                        
                        // 不要引入未定义的值 Don't bring in undefined values
        ...
        // 返回修改后的对象 Return the modified object
        return target;
    };
    
    
    //当只有一个对象时，就把这个对象中的属性和方法扩展到this对象中，这里的this指向jQuery
    jQuery.extend({
        
        // 唯一性，core_version 为jQuery的版本号。\D的意思是：不是数字的，就选中。
        // 因此expando就是jQuery+一段整数。数据缓存，ajax，事件机制都用到了这个。
        ...
        isPlainObject: function (obj) {
            ...
            // 明显的否定  用 tostring 代替 jquery.type
        ...
        // 使用指定的函数过滤数组中的元素，并返回过滤后的数组。invert 是否反转过滤结果
        ...
        // jQuery.support不在核心中使用，但其他项目将它们的属性附加到它，因此它需要存在。
        support: support
    });
...
// Use getAttributeNode to fetch booleans when getAttribute lies
     ...
     */
    var dir = function (elem, dir, until) { // 这个函数很精髓，短短几行代码，支持遍历祖先、所有兄长、所有兄弟！
        var matched = [],                   // 匹配结果
            truncate = until !== undefined;
        
        while ((elem = elem[dir]) && elem.nodeType !== 9) {
            if (elem.nodeType === 1) {// 如果是Element，则放入匹配结果数组
                if (truncate && jQuery(elem).is(until)) {
                    //执行最后的jQuery.is的隐含条件是：until !== undefined && cur.nodeType === 1
                    break;
                }
                matched.push(elem);//放入匹配结果数组
            }
        }
        return matched;
    };
    ...
    */
    ...
    //表示的是html标签;这里面出现了括号的引用。
    var rsingleTag = (/^<([a-z][^\/\0>:\x20\t\r\n\f]*)[\x20\t\r\n\f]*\/?>(?:<\/\1>|)$/i);
    
    // 实现 filter【过滤器】 and not 【过滤取反】 相同的功能
    ...
    // 初始化jquery对象 Initialize a jQuery object
    ...
    // 赋予init方法原因，便于实例化 Give the init function the jQuery prototype for later instantiation
    init.prototype = jQuery.fn;

    // 初始化中心参考 Initialize central reference
    rootjQuery = jQuery(document);
    ...
    jQuery.each({
        ...
        // 获得当前匹配元素集合中每个元素的前面的同胞元素，使用选择器进行筛选是可选的。
        prevAll: function (elem) {
            return dir(elem, "previousSibling");
        },
    
        // 获得每个元素所有跟随的同胞元素，但不包括被选择器、DOM 节点或已传递的 jQuery 对象匹配的元素。
        nextUntil: function (elem, i, until) {
            return dir(elem, "nextSibling", until);
        },
    
        // 获得当前匹配元素集合中每个元素的前面的同胞元素，但不包括被选择器、DOM 节点或 jQuery 对象匹配的元素。
     ...
     * 使用以下参数创建回调列表：
     *
     *	options: 一个可选的空间分隔选项列表，将更改回调列表的行为或更传统的选项对象
     *
     * 默认情况下，回调列表将像事件回调列表一样，可以多次被“fired【解雇】”。
     *
     * Possible options:
     *
     *	once:			将确保回调列表只能被触发一次（如延迟）
     *
     *	memory:			将跟踪以前的值，并调用列表中的任何回调，然后用最新的“memorized【记忆】”值（如延迟）发射列表。
     *
    ...
    jQuery.Callbacks = function (options) {
        ...
        // 如果需要，将 options 从字符串格式转换为对象格式（我们首先检查缓存）
    ...
    jQuery.extend({
        ...
        ready: function (wait) {
            ...
            // 有挂起的 pending 或者已经准备好了。 Abort if there are pending holds or we're already ready
    ...
    Data.prototype = {
        ...
        cache: function (owner) {
            ...
            if (!value) {
                ...
                // 在浏览器中 我们可以接收非元素节点的数据，但是我们不应该这样做。
                if (acceptData(owner)) {
        ...
        access: function (owner, key, value) {
            ...
            //
            //   1. 未指定 key  No key was specified
            //   2. 指定了字符创，但没有指定值  A string key was specified, but no value provided
            //
            // Take the "read" path and allow the get method to determine
            // which value to return, respectively either:
            //
            //   1. 整个的缓存数据  The entire cache object
            //   2. key 中的数据  The data stored at the key
            //
            if (key === undefined ||
            ...
            // 当键不是字符串，或者指定了键和值时，用以下方式设置或扩展（现有对象）：
            //
            //   1. 一个属性对象  An object of properties
            //   2. 一个键值对  A key and value
            //
    ...
    jQuery.fn.extend({
        ...
        // 当某个类型的队列被清空时，得到一个解决方案（默认情况下是FX类型）
    ...
    // 控制元素显示或者隐藏  show 为true时显示，为false时隐藏
    function showHide(elements, show) {
        ...
        for (; index < length; index++) {
            ...
            if (show) {
                ...
                // 因为我们强制对级联隐藏元素进行可视性，
                // 所以在第一个循环中需要立即（或缓慢）检查，
                // 除非我们有一个非空的显示值（内联或即将恢复）。
                if (display === "none") {
    ...
    jQuery.event = {
        ...
        add: function (elem, types, handler, data, selector) {
            ...
            // 如果元素是文本节点（IE下访问文本节点会抛错，因为事件源不能为文本节点），就直接返回。
            // 元素也不能是注释节点。事件类型和事件处理函数不能没有。
            // 如果元素不能添加自定义属性，也直接返回。
            // 如果元素elem可以添加自定义属性，会在jQuery的缓存系统中添加这个元素的缓存对象，并返回，
            // 这时elemData就是缓存系统中，以元素elem为属性的对象。
            if (!elemData) {
                return;
            }
            ...
            // 如果事件处理函数没有唯一的guid属性，就赋值一个
            if (!handler.guid) {
                //jQuery.guid从1开始，累加，因此每一个事件处理函数的guid属性都是唯一的。
                handler.guid = jQuery.guid++;
            }
            
            // Init the element's event structure and main handler, if this is the first
            // 如果此元素elem之前没有绑定过事件处理函数，它在缓存系统中以元素elem为属性的对象的events属性将是undefined
            // 也就是说，如果之前给这个元素elem绑定过事件处理函数，那么这时的events将是一个对象，不会进入if语句
            if (!(events = elemData.events)) {
                events = elemData.events = {};
            }
    
            // 第一次绑定时，为undefined
            // 给此元素elem绑定事件处理函数 function,这个function会处理用户绑定该元素的所有事件处理函数
            // （哪种类型的事件触发，就执行哪种事件绑定的所有事件处理函数）
            ...
            while (t--) {
                ...
                // 并不是所有的事件都能直接使用，比如：火狐下没有mousewheel，需要用DOMMouseScroll冒充
                special = jQuery.event.special[type] || {};
                
                // If selector defined, determine special event api type, otherwise given type
                // 有些事件只需要在事件代理时，需要冒充。
                // 比如：focus，blur，不冒泡的，事件代理使用的就是冒泡机制，所有需要做特殊化处理。
                ...
                // 查看此元素在缓存系统中是否有此事件类型的数组处理函数。第一次绑定此type类型的事件时，是undefined。
                if (!(handlers = events[type])) {
                    
                    //此数组就是用来装载此类事件的事件处理函数的
                    ...
                    // 如果special事件返回 false 仅使用 addEventListener
                    if (!special.setup ||
        ...
        remove: function (elem, types, handler, selector, mappedTypes) {
            ...
            // 如果内部数据不存在，或者内部数据没有events域那么直接返回!
